...
\paragraph{Research Classification.}
From a disciplinary perspective, this work is situated at the intersection of multimodal machine learning, affective computing, and computational social signal processing. Under the ACM Computing Classification System (CCS), it falls primarily within \emph{Computing methodologies}—specifically \emph{machine learning}, \emph{recurrent neural networks}, \emph{computer vision}, and \emph{natural language processing}—with additional relevance to \emph{human-centered computing} through affective computing. From the standpoint of the AMS Mathematics Subject Classification, the work relates most closely to \textbf{68T07} (artificial neural networks and deep learning), \textbf{68T10} (pattern recognition), and \textbf{62M45} (neural networks and related approaches to time series analysis), reflecting the sequential modeling of multimodal behavioral data.

\section{Introduction}

Human romantic attraction plays a central role in social behavior, relationship formation, and well-being. It emerges rapidly during interaction and is shaped by a complex interplay of facial expressions, body movement, vocal characteristics, and spoken language. Psychological research has long emphasized that attraction is not determined by any single cue, but rather by the dynamic integration of multiple behavioral signals unfolding over time. Despite this understanding, computational approaches to modeling attraction have remained limited, often focusing on static features, unimodal cues, or population-level attractiveness ratings rather than dyadic, temporally grounded interaction data.

Recent advances in affective computing and multimodal machine learning have made it possible to automatically extract rich behavioral signals from audiovisual recordings, including facial expressions, gestures, prosody, and language. These methods have been successfully applied to tasks such as emotion recognition, sentiment analysis, and social signal processing. However, predicting romantic attraction poses additional challenges beyond standard affect recognition. Attraction is inherently interpersonal, context-dependent, and only partially observable through behavior. Moreover, it unfolds over short time scales, where subtle temporal patterns and coordination between modalities may carry more information than isolated snapshots.
...
The importance of this problem is twofold. From a scientific perspective, computational models of attraction can contribute to a more precise understanding of how dynamic behavioral cues jointly shape interpersonal judgments, complementing traditional psychological methods based on self-report and manual coding. From a methodological standpoint, attraction prediction represents a challenging testbed for multimodal sequence modeling, as it involves noisy, partially missing signals, strong individual differences, and an outcome that is not perfectly determined by observable behavior. Progress in this area may therefore inform the design of more general models for social and affective inference.

While prior work has demonstrated that individual modalities such as facial expression, body movement synchrony, or vocal characteristics are related to attraction, several gaps remain unresolved. First, many studies analyze these channels in isolation, limiting their ability to capture cross-modal interactions. Second, a substantial portion of the literature relies on static features or global summaries, ignoring the temporal structure of behavior during interaction. Third, computational approaches often target internal affective states (e.g., valence or arousal) rather than explicitly interpersonal outcomes such as dyadic attraction. These limitations motivate the need for models that jointly integrate multiple modalities and explicitly represent their temporal dynamics.

In this article, we address these gaps by proposing a multimodal, sequence-based framework for attraction prediction. Our approach combines automated extraction of facial Action Units, hand and upper-body movement dynamics, acoustic prosodic features, and sentiment information from transcripts, all temporally aligned at the per-second level. These heterogeneous signals are modeled as short behavioral sequences using a stacked Long Short-Term Memory (LSTM) architecture, enabling the model to learn temporal dependencies both within and across modalities. A key aspect of our approach is the use of participant visibility to align audio and visual signals in naturalistic settings, reducing cross-speaker contamination and preserving temporal coherence.

The contributions of this article are threefold. First, we present a practical data collection and preprocessing pipeline for temporally aligned multimodal behavioral analysis in dyadic interactions. Second, we introduce a compact per-second representation that integrates facial, gestural, acoustic, and linguistic features into short sequences suitable for sequential learning. Third, we demonstrate that multimodal sequence modeling enables reliable prediction of romantic interest, outperforming simpler baselines that ignore temporal structure or modality integration.

The remainder of the article is structured as follows. Section~2 describes the multimodal data collection and feature extraction pipeline. Section~3 details the sequence construction, normalization, and LSTM-based modeling methodology. Section~4 reviews related work in affective computing and attraction research. Finally, we discuss the implications of our findings and outline directions for future research.
...
This section presents the empirical results of the proposed multimodal sequence model, interprets their implications, and discusses the conclusions that can be drawn in light of the research questions posed in the Introduction.

\subsection{Model Performance and Validation}

The final stacked LSTM model was evaluated on held-out validation data consisting of temporally segmented 15-second multimodal sequences. Each sequence comprised 28 features aggregated at the per-second level, yielding an input shape of $(1535, 15, 28)$. The model achieved a validation accuracy of $0.92$, with a corresponding binary cross-entropy loss of $0.2021$. Precision and recall were both high, reaching $0.9657$ and $0.9206$, respectively.

The high precision indicates that when the model predicts romantic attraction, it is rarely incorrect, while the strong recall suggests that the majority of attraction cases are successfully identified. Together, these metrics indicate a balanced classifier that does not rely on trivial decision rules or degenerate predictions. The close alignment between accuracy, precision, and recall further suggests that performance is not driven by class imbalance alone.

\subsection{Prediction Distribution and Behavioral Balance}

An analysis of the prediction distribution shows that the model produced 204 positive (attracted) and 96 negative (not attracted) predictions, closely mirroring the ground-truth distribution of 214 positive and 86 negative samples. This correspondence indicates that the classifier does not exhibit a systematic bias toward either class and that it preserves the underlying structure of the data. Importantly, this balance supports the interpretation that the model has learned discriminative patterns in the multimodal behavioral sequences rather than exploiting label frequency artifacts.

\subsection{Interpretation of Results}

These results provide evidence that short multimodal behavioral sequences contain sufficient information to predict romantic attraction with high reliability. In particular, the model’s strong performance supports the hypothesis that attraction is communicated through the temporal co-occurrence of facial expressions, bodily movement, vocal characteristics, and linguistic sentiment, rather than through static or unimodal cues alone. By operating on 15-second windows, the model captures fine-grained temporal dynamics that are likely to be missed by approaches relying on global summaries or single-modality features.

While direct numerical comparisons with prior work are difficult due to differences in datasets, target variables, and experimental protocols, the achieved performance is comparable to or exceeds reported results in multimodal affect recognition and social judgment tasks that rely on similar sequential architectures. Unlike many existing approaches, however, the present model predicts an explicitly interpersonal outcome at the dyadic level, rather than an internal affective state such as emotion or sentiment. This distinction underscores the practical and conceptual relevance of the results.

\subsection{Answering the Research Questions}

The obtained results directly address the research questions posed in this article. First, the high validation accuracy and balanced precision–recall profile demonstrate that short multimodal behavioral sequences can indeed predict binary romantic attraction more effectively than static or unimodal representations, supporting the motivation for temporal and multimodal modeling. Second, the success of the integrated feature set suggests that informative cues arise from the combination of facial, bodily, vocal, and linguistic signals when modeled jointly over time. Third, the results indicate that sequential neural models can capture meaningful interpersonal outcomes, even when these outcomes are only partially observable from behavior and subject to individual variability.

\subsection{Validity of Conclusions}

Several aspects of the experimental design support the validity of these conclusions. Temporal segmentation and per-second aggregation ensure that predictions are based on dynamic interaction patterns rather than isolated frames. Participant-level data splitting prevents information leakage between training and validation sets, promoting generalization to unseen individuals. Furthermore, deterministic handling of missing modalities through zero filling avoids introducing artificial correlations via imputation. Together, these design choices strengthen confidence that the reported performance reflects learned multimodal behavioral patterns rather than artifacts of preprocessing or evaluation.

At the same time, it is important to acknowledge that romantic attraction is influenced by factors beyond observable behavior, including prior preferences, context, and unmeasured interpersonal dynamics. Consequently, perfect prediction is neither expected nor theoretically plausible, and the reported performance should be interpreted as an upper bound constrained by the observable signals available to the model.

\subsection{Conclusions and Future Work}

In conclusion, this work demonstrates that multimodal, sequence-based modeling provides a viable and effective approach to predicting romantic attraction from short segments of dyadic interaction. By integrating facial expressions, body movement, vocal prosody, and language into a unified temporal framework, the proposed method advances computational approaches to interpersonal affect and social judgment.

The presented approach opens several directions for future research. These include exploring attention-based or transformer architectures to improve interpretability and long-range temporal modeling, conducting systematic ablation studies to quantify the contribution of individual modalities, and extending the framework to multi-class or continuous attraction measures. Additionally, applying the model to larger and more diverse datasets would enable a deeper examination of generalization across cultures, interaction contexts, and communication settings. Together, these directions point toward a richer computational understanding of attraction as a dynamic, multimodal social process.
