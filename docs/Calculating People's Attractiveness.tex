...
Stacked LSTM (sequences) & 0.920 & 0.966 & 0.921 \\
\hline
\textbf{Absolute Improvement} & +0.173 & +0.050 & +0.211 \\
\textbf{Relative Improvement} & +23.2\% & +5.5\% & +29.7\% \\
\hline
\end{tabular}
\end{table}

\textbf{Regarding RQ1 (predictive power):} The model achieved a validation accuracy of 92\% with precision of 96.6\% and recall of 92.1\% on the positive class (attraction), demonstrating that short multimodal behavioral sequences do contain predictive information about binary romantic attraction \citep{2017multimodalmachinelearningsurvey, kraack2023}.

\textbf{Regarding RQ2 (temporal modeling):} To assess the value of temporal sequence modeling, we compared the LSTM against a logistic regression baseline using time-averaged features. The baseline achieved an accuracy of 74.7\%, precision of 91.6\%, and recall of 71.0\%. The LSTM model outperformed the baseline by 17.3 percentage points in accuracy (23.2\% relative improvement) and 21.1 percentage points in recall (29.7\% relative improvement), confirming that temporal dependencies across the 15-second windows provide substantial predictive information beyond static feature summaries \citep{schmidt2019, lebois2018}. The particularly large improvement in recall indicates that the LSTM is better at detecting subtle attraction cues that unfold over time, supporting the hypothesis that attraction signals are inherently dynamic and sequential in nature \citep{chang2021}.

\textbf{Regarding RQ3 (multimodal integration):} While the integrated feature set combining facial, bodily, vocal, and linguistic signals shows promise \citep{didonato2023, schirmer2025}, systematic ablation studies are needed to quantify the relative contribution of each modality and confirm the advantage of multimodal integration over single-channel approaches. 

However, several limitations should be noted.  First, the dataset exhibits class imbalance (71\% attracted vs. 29\% not attracted), which may influence model predictions. Second, the relatively small validation set (300 samples) limits the strength of generalization claims.  Third, we have not yet conducted the ablation studies necessary to definitively answer RQ3.
